...
def _runner(tmp_path: Path, result: _TestRunResult):
...
def test_valid_baseline_freezes_f0(tmp_path: Path) -> None:
    ...
    assert runner.snapshot_store.best_contents == runner.snapshot_store.baseline_contents
...
def test_valid_empty_baseline_stops_success(tmp_path: Path) -> None:
    ...
    runner = _runner(tmp_path, _TestRunResult(exit_code=0, cases=()))
